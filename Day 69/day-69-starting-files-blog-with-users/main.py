...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        hash_and_salt_pass = generate_password_hash(password=register_form.password.data, method="pbkdf2:sha256",salt_length=8)
        new_user = User(email=register_form.email.data,password=hash_and_salt_pass, name=register_form.name.data)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if not user:
            ...
            flash("Passwords incorrect, please try again.")
            return redirect(url_for('login'))
        else:
            login_user(user)
            return redirect(url_for("get_all_posts"))
